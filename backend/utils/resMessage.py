...
def res_online(client, userdata, msg):
    ...
    logger.info(StaticData.device_list)
...
def res_start(client, userdata, msg):
    ...
    if deviceInform["stat"] == "on":
        ...
        logger.info("{0} start sampling!".format(deviceInform["deviceId"]))
...
def res_showdata(client, userdata, msg):
    ...
    if data_type == "wav":
        ...
        data = np.array(
            [math.sqrt(data_total[0] * data_total[0] + data_total[1] * data_total[1])]
        )
        # 实时呼吸需要用到真实数据，不能简单处理
