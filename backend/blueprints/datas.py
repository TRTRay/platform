...
def show_data():
    ...
    if deviceInform['devType'] == 'Speaker':
        ...
        # 取出部分
        run_data_len = math.floor(len(StaticData.data_slice[data_key]) / seg_len / recorderChannels) * seg_len * recorderChannels
        # 取出数据（防止清空缓存导致临时数据失效要进行拷贝）
        runtime_list = copy.copy(StaticData.data_slice[data_key][0:run_data_len])
