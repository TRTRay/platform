...
def device_ctl():
    ...
    if request.method == 'GET':
        ...
        client.publish('/broker/request/deviceInform', payload=json.dumps({'message':'Broker request for device information'}))
    ...
    # pub_topic = '/broker/{devType}/{deviceId}/reboot, payload = None
        ...
        if operation == 'update':
            client.publish(pub_topic, payload=json.dumps({'params': temp['params'], 'message':'Broker request for changing device params'}))
            deviceInform['params'] = temp['params']
        else:
            client.publish(pub_topic, payload=json.dumps({'message': 'Broker request for rebooting'}))
